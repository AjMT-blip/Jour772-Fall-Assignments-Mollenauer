...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer: There are 24 rows and 18 columns.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: I'm not sure whether this is because I might've run this code incorrectly, but I am not observing any changes from the last chart to this one. Each axis from the last to the current chart appears to be the same, and the values/counties measured here are in the same order with their numbers also appearing to be the same.**
