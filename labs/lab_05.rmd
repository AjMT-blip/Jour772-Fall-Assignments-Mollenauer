...
```{r}
calls <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |>
  clean_names() |>
...
A4. I see that one great story idea would be focusing on how big of a percentage of these overdoses took place on weekdays. The reason I say this is because in the context of days of the week, many would assume these drug-related tragedies might commonly be on weekend days/nights like Fridays and Saturdays; but if I could identify a clear prevalence of these incidents on days like Mondays-Thursdays, I might have a strong indicator of overdoses being frequently due to matters of addiction or suicide rather than results of excessive partying. That correlation of course wouldn't be perfect and would be rooted in an assumption that I might expect true but that might not be as widely accurate as I suspect. But a correlation nonetheless is at least telling. On the flip-side, could it be that most of these deaths are actually due to partying gone out of control? In which case, the angle is considerably different and would examine how and why these are happening at parties. For us as college students, that's a story with major hyper-local value even if it speaks to a more pervasive trend.
