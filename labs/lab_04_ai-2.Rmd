...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "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")
```
...
maggies_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure, including headers, with no yapping and do not include the firstsentence of the response: title, year, role, director, role, co-star and awards", text),
...
course_response <- axolotr::ask(
  prompt = paste("can you explain without yapping: what the r_code_example does to the 
umd_courses csv file", r_code_example),
  model = "llama-3.1-8b-instant"
)
course_response
...
```{r}
#Add code below to talk to Groq and display its response
your_r_code_lab_response <- axolotr::ask(
  prompt = paste("Is this code correct", r_code_example, your_r_code_lab),
  model = "llama-3.1-8b-instant"
)
your_r_code_lab_response
...
idea_response <- axolotr::ask(
  prompt = "I have a CSV file of daily data on crash incidents in montgomery county that gives local case number, acrs report type, crash date and time, as well as collision type. Using R and the tidyverse, suggest some ways that I could find patterns in the data. Use the new-style pipe operator (|>) in any code examples",
  model = "llama-3.1-8b-instant"
)
...
**Answer: I found Groq's response here to be confusing. It was too formulaic for my liking, as it read almost like a recipe like, 'do x, then do y, then...'. I know that's somewhat to be expected since it's a computer, but I feel like its explanation would have been much stronger with some form of conceptual directions. The explanation seemed more clear when it began to use terminology we are learning about and using repeatedly, such as "mutate."**
...
#Add code below to talk to Groq and display its response
idea_response <- axolotr::ask(
  prompt = "Please write me a 500-word article about a car crash at the intersection between Knox Road and Regents Drive",
  model = "llama-3.1-8b-instant"
)
print(idea_response)
...
**Answer: Main thing I can think of is that this output/explanation as it's presented feels very chaotic and jumbled and unorganized. I find it informative to a degree, but it's also hard to interpret because it just feels messy and doesn't seem to have true rhyme or reason. The / marks also make it difficult to retain/mentally compute this info smoothly.**
