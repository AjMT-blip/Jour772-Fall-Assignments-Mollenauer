...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer The output I see shows what appears to be a moderate level of consistency among regions within Maryland. For example, several regions toward the middle are highlighted in black and are all bordering each other. The same is true near that cluster, as several bordering each other are purple. There are also a few smaller clusters of two of the same color bordering each other, so overall I'd say there's at least some level of regional consistency, which I'm not sure would establish strong-enough correlation, but I see some connection, regionally.**
